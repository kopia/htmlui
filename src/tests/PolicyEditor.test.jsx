...
it("e2e", async () => {
  ...
  serverMock
    .onGet("/api/v1/policy?userName=some-user&host=some-host&path=some-path")
    .reply(200, {
      retention: {
        keepLatest: 33,
      },
  ...
  serverMock
    .onPost(
      "/api/v1/policy/resolve?userName=some-user&host=some-host&path=some-path",
      {
        updates: {
          retention: {
            keepLatest: 33,
    ...
    .reply(200, {
      ...
      upcomingSnapshotTimes: [ust1, ust2],
    });
  ...
  serverMock
    .onPut("/api/v1/policy?userName=some-user&host=some-host&path=some-path", {
      retention: {
        keepLatest: 44,
      },
    })
  ...
  await waitFor(() =>
    expect(getByTestId("effective-retention.keepHourly").value).toBe("45"),
  );
  expect(getByTestId("effective-retention.keepLatest").value).toEqual("33");

  await waitFor(() =>
    expect(getByTestId("definition-retention.keepLatest").innerHTML).toContain(
      `Directory: some-user@h1:some-path`,
    ),
  );
  await waitFor(() =>
    expect(getByTestId("definition-retention.keepMonthly").innerHTML).toContain(
      "Directory: u1@h1:p1",
    ),
  );
  ...
  await waitFor(() =>
    expect(getByTestId("upcoming-snapshot-times").innerHTML).toEqual(
      expectedUpcomingSnapshotTimes,
    ),
  );
  ...
  await waitFor(() =>
    expect(getByTestId("effective-retention.keepLatest").value).toBe("44"),
  );
  await waitFor(() =>
    expect(getByTestId("definition-retention.keepLatest").innerHTML).toEqual(
